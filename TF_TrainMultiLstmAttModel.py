...
modelSaveFileTemp = "result_temp/savedModel.model"
modelSaveFileTemp = "result1_temp/savedModel.model"
...
output_loss_file = "result1/loss_print.log"
...
log_loss_file = open(output_loss_file, 'a')
# loss 日志
loss_list = []
        ...
        for i in range(lrDownCount):
            ...
            for epoch in range(epochs):
                for question, trueAnswer, falseAnswer in zip(tqs, tta, tfa):
                    ...
                    loss_list.append("step:%d, loss: %.8f, time: %.8f" % (step, loss, timeUsed))
            saver.save(sess, modelSaveFile)
            try:
                if i == 2:
                    saver.save(sess, modelSaveFileTemp)
            except:
                print("save temp model faild")
            try:
                log_loss_file.write('\n'.join(loss_list) + "\n-------------------------------")
            except:
                log_loss_file.write("写出错了")
            loss_list = []
